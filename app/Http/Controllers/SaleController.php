            ...
            foreach ($validated['items'] as $item) {
                ...
                if ($validated['sale_status'] === 'completed') {
                    ...
                    if ($availableStock < $item['quantity']) {
                        throw new \Exception(
                            "❌ Insufficient stock for '{$product->name}' in selected warehouse. Available: {$availableStock}, Requested: {$item['quantity']}"
                        );
                    }
                }
                ...
                if ($validated['sale_status'] === 'completed') {
                    ...
                    Log::info('Warehouse stock deducted', [
                        ...
                        'remaining_in_warehouse' => \App\Models\ProductWarehouse::getAvailableStock(
                            $validated['warehouse_id'],
                            $item['product_id']
                        ),
                    ]);
